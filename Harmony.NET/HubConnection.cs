...
namespace Harmony {
	...
	internal class HubConnection {
		...
		public async Task Connect(string hostname, string hubRemoteId) {
            ...
            WebSocket = new WebSocket(FullUrl, onMessage: OnMessage);
		...
		public async Task Disconnect() {
			await this.WebSocket.Close(CloseStatusCode.Normal, "Closed");
		}
	    ...
	    {
	        var message = await this.ReceiveStringMessage();
	        return JsonConvert.DeserializeObject<Response<T>>(message);
	    }
	    ...
	    private async Task OnMessage(MessageEventArgs arg)
	    {
	        buffer.Post(await arg.Text.ReadToEndAsync());
	    }
		...
		private async Task SendString(string data) {
			...
			await this.WebSocket.Send(Encoding.UTF8.GetBytes(data));
		}
	}
}